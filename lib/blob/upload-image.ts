...
export async function uploadCandidateImageToBlob(params: {
  venueId: string;
  candidateId: string;
  sourceUrl: string;
  contentType: string;
...
export async function uploadArtworkImageToBlob(params: {
  artworkId: string;
  candidateId: string;
  sourceUrl: string;
  contentType: string;
  ...
  const hash = createHash("sha256").update(params.bytes).update("|").update(params.sourceUrl).digest("hex");
  const extension = extensionForContentType(params.contentType);
  const path = `artworks/ingest/${params.artworkId}/${params.candidateId}/${hash}.${extension}`;
  const blob = await uploadToBlob(path, Buffer.from(params.bytes), {
    access: "public",
    contentType: params.contentType,
    cacheControlMaxAge: 31536000,
...
export async function uploadVenueImageToBlob(params: {
  ...
  const hash = createHash("sha256").update(params.bytes).update("|").update(params.sourceUrl).digest("hex");
  const extension = extensionForContentType(params.contentType);
  const path = `venues/ingest/${params.venueId}/${hash}.${extension}`;
  const blob = await uploadToBlob(path, Buffer.from(params.bytes), {
    access: "public",
    contentType: params.contentType,
    cacheControlMaxAge: 31536000,
