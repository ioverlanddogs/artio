...
function makeDeps(overrides?: Partial<Parameters<typeof handleStripeWebhook>[1]>) {
  ...
  const deps: Parameters<typeof handleStripeWebhook>[1] = {
    ...
    findRegistrationByPaymentIntentId: async () => ({ id: "reg-1", status: "PENDING", guestEmail: "guest@example.com" }),
    findRegistrationById: async () => ({ id: "reg-1", status: "PENDING", guestEmail: "guest@example.com" }),
