...
import {
  fetchNearbyEvents,
  fetchNearbyVenues,
  normalizeNearbyNumber,
} from "@/lib/nearby-client-fetch";
...
export function NearbyClient({
  ...
  const filters = useMemo(
    () => parseNearbyFilters(searchParams),
    [searchParams],
  );
  ...
  const loadNearby = useCallback(
    async ({
      ...
      mode?: "reset" | "append";
      cursor?: string | null;
      override?: { lat: string; lng: string };
    } = {}) => {
      ...
      try {
        ...
        if (!eventsResult.ok || !venuesResult.ok) {
          const nextError = !eventsResult.ok
            ? eventsResult.error
            : !venuesResult.ok
              ? venuesResult.error
              : "Unable to load nearby results.";
  ...
  useEffect(() => {
    ...
    const stored = resolveNearbyView(
  ...
  useEffect(() => {
    if (canSearch) void loadNearby({ mode: "reset" });
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          setForm((prev) => ({
            ...
            radiusKm: safeRadiusKm(prev.radiusKm),
          }));
          if (isAuthenticated) {
          ...
          await loadNearby({
            mode: "reset",
            override: { lat: nextLat, lng: nextLng },
          });
      ...
      (error) => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
          ...
          setMessage(
            "Unable to access your location. Search for a place instead.",
          );
        }
        setIsLocating(false);
      },
    ...
    <div className="space-y-5">
      <div className="rounded-xl border border-border bg-card p-4">
        <LocationPreferencesForm
          ...
          saveButtonLabel={
            isAuthenticated ? "Save location" : "Use this location"
          }
          onSave={async (payload) => {
            setForm({
              ...
              radiusKm: safeRadiusKm(String(payload.radiusKm)),
            });
          ...
          afterSave={(next) =>
            setForm((prev) => ({
              ...prev,
              ...next,
              radiusKm: safeRadiusKm(next.radiusKm),
            }))
          }
        />
        <div className="mt-3 rounded-md border p-3">
          <p className="text-sm text-muted-foreground">
            Use your device location for faster nearby results.
          </p>
          <button
            type="button"
          ...
          <div className="mt-3">
            <SaveSearchButton
              ...
              params={{
                ...
                tags: filters.tags,
                days: filters.from || filters.to ? undefined : filters.days,
                from: filters.from || undefined,
                to: filters.to || undefined,
                sort: filters.sort,
                view,
              }}
              defaultName={`Nearby: ${form.locationLabel || "Current location"} (${form.radiusKm || "25"}km)`}
            />
          </div>
        ) : null}
      </div>
      ...
      {message ? (
        <ErrorCard
          message={message}
          onRetry={() => void loadNearby({ mode: "reset" })}
        />
      ) : null}
        ...
        eventItems.length === 0 && venueItems.length === 0 ? (
          canSearch ? (
            <EmptyState
              ...
              actions={[
                {
                  label: isAuthenticated ? "Manage location" : "Sign in",
            ...
            <div className="rounded-lg border p-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Nearby uses your device location to compute distance.
              </p>
                  ...
                  {eventItems.map((item, idx) => (
                    ...
                    >
                      <EventCard
                        ...
                        imageUrl={item.primaryImageUrl}
                        distanceLabel={toKmLabel(item)}
                        badges={(item.tags ?? []).map((tag) => tag.slug)}
                      />
                    </div>
                  ))}
                </section>
                {nextCursor ? (
                  <div className="pt-2">
                    <button
                      ...
                      className="rounded border px-3 py-1 text-sm"
                      onClick={() =>
                        void loadNearby({ mode: "append", cursor: nextCursor })
                      }
                      disabled={isLoading}
                    >
