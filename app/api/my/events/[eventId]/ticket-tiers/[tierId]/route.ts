...
import { handleDeleteTicketTier, handlePatchTicketTier } from "@/lib/ticket-tier-route";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ eventId: string; tierId: string }> }) {
  return handlePatchTicketTier(req, params, deps);
}

export async function DELETE(req: NextRequest, { params }: { params: Promise<{ eventId: string; tierId: string }> }) {
  return handleDeleteTicketTier(req, params, deps);
}
