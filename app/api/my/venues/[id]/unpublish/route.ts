...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return handleVenueSelfPublish(req, { venueId: parsedId.data.id, isPublished: false }, {
    ...
    updateVenuePublishState: (venueId, isPublished) => db.venue.update({ where: { id: venueId }, data: { isPublished }, select: { id: true, slug: true, name: true, description: true, featuredAssetId: true, city: true, country: true, websiteUrl: true, deletedAt: true, isPublished: true } }),
    logAdminAction,
  });
}
