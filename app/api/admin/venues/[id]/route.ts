...
import { handleAdminEntityGet, handleAdminEntityPatch } from "@/lib/admin-venues-route";
import { requireAdmin } from "@/lib/admin";

export const runtime = "nodejs";

export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return handleAdminEntityGet(req, await params, { requireAdminUser: requireAdmin, appDb: db });
}
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error("admin_venues_id_unexpected_error", {
