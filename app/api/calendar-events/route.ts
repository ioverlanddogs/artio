...
export async function handleCalendarEventsGet(req: NextRequest, deps: CalendarEventDeps = {
  getUser: guardUser,
  ...
  const parsed = calendarEventsQuerySchema.safeParse(paramsToObject(req.nextUrl.searchParams));
  if (!parsed.success) {
